...
export default class AddRice extends Component {
  constructor(props) {
    super(props);
  ...
  componentWillMount() {
    if (localStorage.rice) {
      this.setState({
        allRecipes: JSON.parse(localStorage.getItem("rice")),
      });
    }
  }

  componentDidMount() {
    localStorage.setItem("rice", JSON.stringify(this.state.allRecipes));
    $(document).ready(() => {
      $('.modal').modal();
    });
  }
  ...
  addRecipe = (newRecipe) => {
    ...
    localStorage.setItem("rice", allRecipesLocal);
    this.setState({
      allRecipes,
      editRecipe: true,
    });
