...
export default class AddNewRecipes extends Component {
  constructor(props) {
    super(props);
  ...
  componentWillMount() {
    if (localStorage.newrecipes) {
      this.setState({
        allRecipes: JSON.parse(localStorage.getItem('newrecipes')),
      });
    }
  }
  ...
  componentDidMount() {
    localStorage.setItem('newrecipes', JSON.stringify(this.state.allRecipes));
    $(String).ready(() => {
      $('.modal').modal();
    });
  }
  ...
  addRecipe = (newRecipe) => {
    ...
    localStorage.setItem('newrecipes', allRecipesLocal);
    this.setState({
      allRecipes,
      editRecipe: true,
    });
  ...
  deleteRecipe = () => {
    ...
    localStorage.setItem('rice', updatedRecipesLocal);
    this.setState({
  ...
  updateRecipe = (position, updatedRecipe) => {
    ...
    localStorage.setItem('newrecipes', updatedRecipesLocal);
    this.setState({
