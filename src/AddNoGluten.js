...
export default class AddNoGluten extends Component {
  constructor(props) {
    super(props);
  ...
  componentWillMount() {
    if (localStorage.NoGluten) {
      this.setState({
        allRecipes: JSON.parse(localStorage.getItem("NoGluten")),
      });
    }
  }

  componentDidMount() {
    localStorage.setItem("NoGluten", JSON.stringify(this.state.allRecipes));
    $(document).ready(() => {
      $('.modal').modal();
    });
  }
  ...
  addRecipe = (newRecipe) => {
    ...
    localStorage.setItem("NoGluten", allRecipesLocal);
    this.setState({
      allRecipes,
      editRecipe: true,
    });
  ...
  deleteRecipe = () => {
    ...
    localStorage.setItem("NoGluten", updatedRecipesLocal);
    this.setState({
